...
export function ContentCard({ title, summary, tag, platform, link }: ContentCardProps) {
  ...
  return (
    <motion.div
      className="p-6 rounded-2xl bg-white border border-brand-border/50 shadow-sm h-full flex flex-col hover:shadow-md hover:border-brand-gold/40 transition-all duration-300"
    ...
    >
      ...
      <h4 className="text-base lg:text-lg font-semibold text-brand-heading mb-2 leading-snug line-clamp-2">{title}</h4>
      <p className="text-brand-body/70 text-sm mb-5 leading-relaxed flex-grow line-clamp-3">{summary}</p>
      ...
      <div className="flex items-center gap-4 pt-4 border-t border-brand-border/30">
        {link ? (
          <a
            ...
            className="flex items-center gap-1.5 text-sm font-medium text-brand-accent hover:text-brand-body transition-colors group"
          >
          ...
          <span className="text-sm text-brand-body/40 italic">Em breve</span>
        )}
      </div>
    </motion.div>
  );
